# Engenharia de Requisitos - Gopher
...
| **RF05** | Persistência de Especificações         | As configurações dos agentes devem ser armazenadas localmente em `.gopher/agents/*.json`.                                     |
...
| **RF10** | Prefixo de Mensagens do Gopher         | Toda mensagem exibida ao usuário deve iniciar com `🐹:`.                                                                      |
...
| **RF15** | Diretório de Output por Agente         | O sistema deve manter a estrutura `.gopher/outputs/<agente>/` para armazenar os outputs de conclusão de cada agente.          |
...
| **RF23** | Instalador Automático do Binário       | O sistema deve disponibilizar um instalador automático que gere o binário `gopher` dentro da pasta `bin/` do projeto.         |
